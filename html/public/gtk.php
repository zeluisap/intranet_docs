...
class JanelaProgresso extends GtkWindow {
    ...
    public function fileOpen() {
        $dialog = new GtkFileChooserDialog("Selecione o Arquivo: ", null, Gtk::FILE_CHOOSER_ACTION_OPEN, array(Gtk::STOCK_OK, Gtk::RESPONSE_OK,
    ...
    public function corrige_telegone($telefone) {
        ...
        switch (strlen($telefone)) {
            case 7: $telefone = '9' . $telefone; break;
        }
        return $telefone;
    }
        ...
        if ($linhas) {
            ...
            foreach ($linhas as $linha) {
                if (trim($linha["novo_bairro"])) {
                    $linha["novo_bairro"] = utf8_encode($linha["novo_bairro"]);
                    ...
                    if ($obj->cpfcnpj) {
                        ...
                        if ($filter->isValid($obj->cpfcnpj)) {
                            ...
                            if (!$pf) {
                                ...
                                $dados["nome"] = $obj->nome;
                                if (trim($obj->ci)) { $dados["identidade_numero"] = $obj->ci; }
                                if (trim($obj->ciemissor)) { $dados["identidade_orgao_expedidor"] = $obj->ciemissor; }
                                if (trim($obj->email)) { $dados["email"] = $obj->email; }
                                $pf = $tb->createRow();
                                $pf->setFromArray($dados);
                                $pf->save();
                            }
                            if (!$pf->getId()) {
                                echo "nenhuma pessoa fisica cadastrada"; Zend_Debug::dump($obj); die();
                            }
                            $pessoa = $pf->pega_pessoa();
                            if (trim($obj->cnh)) {
                                $pm = $pf->pegaPessoaMotorista();
                                if (!$pm) {
                                    $dados_cnh = array("id_pessoa_fisica" => $pf->getId(),
                                                       "cnh_numero" => $obj->cnh);
                                    if ($obj->cnhcat) {
                        ...
                        if (isset($pessoa) && $pessoa) {
                            $dados = array();
                            if (trim($obj->endereco)) { $dados["logradouro"] = $obj->endereco; }
                            if (trim($obj->cep)) { $dados["cep"] = $obj->cep; }
                            if (trim($obj->enderecouf) && trim($obj->municipio) && trim($obj->bairro)) {
                                ...
                                if ($ufs && count($ufs)) {
                                    ...
                                    if ($muns && count($muns)) {
                                        ...
                                        $municipio->setFromArray(array("id_uf" => $uf->getId(), 
                            ...
                            if (trim($obj->foneresdencial)) { 
                                ...
                                if (!$rs  || !$rs->rowCount()) {
                                    ...
                                    if ($tt) {
                                        ...
                                        $fone->setFromArray(array("ddd" => "96", 
            ...
            while ($obj = $stmt->fetch(Zend_Db::FETCH_OBJ)) {
                ...
                if ($al) {
                    if (trim($obj->codigo) && trim($obj->descartigo)) {
                        ...
                        $dados = array("descricao" => $obj->descartigo, 
            ...
            while ($obj = $stmt->fetch(Zend_Db::FETCH_OBJ)) {
                ...
                //if ($obj->chassi) {
                    ...
                    if (trim($obj->municipio) && trim($obj->uf)) {
                        ...
                        $obj->municipio = $filter->filter(utf8_encode($obj->municipio));
                        $sql = $db->select();
                        ...
                        if ($stmt_municipio && $stmt_municipio->rowCount()) {
                            ...
                            $tb_municipio = new TbMunicipio();
                            $municipio = $tb_municipio->createRow();
                            $municipio->setFromArray(array("descricao" => $obj->municipio,
                                                           "id_uf" => $uf->getId()));
                    ...
                    if (!$errors) {
                        ...
                        Zend_Debug::dump($obj);
                        Zend_Debug::dump($errors); die("erro");
                    }
                //}
                        ...
                        if ($stmt1 && $stmt1->rowCount()) {
                            ...
                            if (!$errors) {
                                ...
                                if ($pessoa) {
                                    ...
                                    $proprietario->setFromArray(array("id_transporte" => $transporte->getId(), 
                                ...
                                if ($stmt2 && $stmt2->rowCount()) {
                                    while ($obj_veiculo = $stmt2->fetch(Zend_Db::FETCH_OBJ)) {
                                        ...
                                        if ($veiculo) {
                                            $tvs = $tb_tv->listar(array("id_transporte" => $transporte->getId(),
            ...
            if ($rs && count($rs)) {
                ...
                if ($stmt && $stmt->rowCount()) {
                    ...
                    while ($obj = $stmt->fetch(Zend_Db::FETCH_OBJ)) {
                        ...
                        $transportes = $tb_transporte->listar(array("filtro_id_transporte_grupo" => $tg->getId(),
                                                                    "filtro_codigo" => $codigo));
                        if ($transportes && count($transportes)) {
                            $transporte = $transportes->current();
                            $dados = array("id_transporte" => $transporte->getId(),
                            ...
                            switch ($obj->licencasit) {
                                case 1: $sss = $tb_sss->getPorChave("PG"); break;
                                case 2: $sss = $tb_sss->getPorChave("CA"); break;
                            }
                            if ($sss) {
                                $dados = array("data_solicitacao" => $obj->licencadt,
                                               "id_servico_transporte_grupo" => $stg->getId(),
            ...
            while ($obj = $stmt->fetch(Zend_Db::FETCH_OBJ)) {
                ...
                if ($pf) {
                    ...
                    try {
                        ...
                        if (!$cnh_categoria) {
                            Zend_Debug::dump($obj); 
                            Zend_Debug::dump("categoria inv�lida!"); die();
                        }
                        $uf = $tb_uf->getPorSigla($obj->cnhuf);
                        if (!$uf) {
                            Zend_Debug::dump("uf inv�lida!"); die();
                        }
                        $pm->setFromArray(array("cnh_numero" => $obj->cnhnumero,
                        ...
                        if (!$errors) {
                            ...
                            if ($obj->matricula) {
                                $tg = $tb_tg->getPorChave("TX");
                                if ($tg) {
                                    $rs_motorista = $tb_motorista->listar(array("filtro_id_transporte_grupo" => $tg->getId(),
                                    ...
                                    $motorista->setFromArray(array("matricula" => $obj->matricula,
                                    ...
                                    if (!$errors) {
                                        ...
                                        if ($tpt && $tps) {
                                            if ($obj->fktaxi) {
                                                $codigo = "RR" . Escola_Util::zero($obj->fktaxi, 5);
                                                $rs_transporte = $tb_transporte->listar(array("filtro_id_transporte_grupo" => $tg->getId(),
                                                                                              "filtro_codigo" => $codigo));
                                                if ($rs_transporte && count($rs_transporte)) {
                                                    ...
                                                    if (!($rs_tp && count($rs_tp))) {
                                                        $tp = $tb_tp->createRow();
                                                        $tp->setFromArray(array("id_transporte" => $transporte->getId(),
                                                                                "id_pessoa" => $pf->id_pessoa,
                    ...
                    try {
                        ...
                        if ($stmt1 && $stmt1->rowCount()) {
                            ...
                            if (!$errors) {
                                ...
                                Zend_Debug::dump($errors); 
                                Zend_Debug::dump("erros d concessao"); die();
                            }
    ...
    public function roda_relatorio_dtce() {
        $f = fopen("/var/www/intranet/intranet//relatorio_dtce.csv", "w+");
