...
class DocumentoController extends Escola_Controller_Logado {
	...
	public function toformAction() {
            ...
            if (isset($dados["id_documento_tipo"]) && $dados["id_documento_tipo"]) {
                ...
                if ($doc) {
                    $this->view->result = utf8_encode($doc->toForm($this->view, $funcionario));
                }
            }
	}
	...
	public function infoAction() {
		...
		if (isset($dados["id_documento"]) && $dados["id_documento"]) {
			...
			if ($doc) {
				$this->view->doc = utf8_encode($doc->toString());
			}
		}
	}
					...
					foreach ($registros as $registro) {
						...
						if ($registro) {
							if (!$registro->vinculado() && !$doc_principal->possui($registro)) {
								...
								$obj->data_hora = utf8_encode(Escola_Util::formatData($registro->data_criacao)) . " " . $registro->hora_criacao;
								$obj->tipo = utf8_encode($registro->findParentRow("TbDocumentoTipo")->toString());
								$obj->numero = utf8_encode($registro->mostrarNumero());
								$obj->resumo = utf8_encode($registro->resumo);
								$items[] = $obj;
							}
						}
					}
	...
	public function indexAction() {
		...
		$button->addFromArray(array("titulo" => "Pesquisar",
                ...
                if ($this->view->registros && count($this->view->registros)) {
                    $button->addFromArray(array("titulo" => "Encaminhar",
                                                "onclick" => "encaminhar()",
                                                "img" => "icon-share",
                                                "id" => "btn_encaminhar",
                                                "params" => array("id" => 0)));
                    ...
                    try {
                        ...
                        if ($files && is_array($files) && count($files)) {
                            ...
                            */
                            ...
                            foreach ($files as $arquivo_idc => $arquivo) {
                                $contador++;
                                $row = $tb->createRow();
                                $dados = array("arquivo" => $arquivo, "legenda" => "Arquivo Importado na Cria��o!");
                        ...
                        if ($errors) {
                            ...
                            if ((isset($dados["tipo_destino"]) && $dados["tipo_destino"]) && (isset($dados["id_destino"]) && $dados["id_destino"])) {
                                ...
                                $mt = $tb->getPorChave("E");
                                $flag = array("id_movimentacao_tipo" => $mt->getId(),
		...
		if ($registro->getId()) {
			...
			if ($funcionario) {
				...
				if ($flag) {
					...
					$this->_flashMessage("FALHA AO EXECUTAR OPERA��O!");
				}
			}
		}
		$this->_redirect($this->_request->getControllerName() . "/index");
		...
		if ($registro) {
			...
			if ($funcionario) {
				if ($registro->habilitaEncaminhar($funcionario)) {
					...
					$this->_flashMessage("FALHA AO EXECUTAR OPERA��O, PERMISS�O NEGADA!");
					$this->_redirect("documento/index");
				}
			} else {
				$this->_flashMessage("FALHA AO EXECUTAR OPERA��O, NENHUM FUNCION�RIO LOGADO!");
				$this->_redirect("documento/index");
			}
		} else {
			...
			if ($funcionario) {
				if ($registro->habilitaTornarProcesso($funcionario)) {
					...
					if ($doc) {
						...
						$this->_flashMessage("FALHA AO EXECUTAR OPERA��O, CHAME O ADMINISTRADOR!");
					}
				} else {
					$this->_flashMessage("FALHA AO EXECUTAR OPERA��O, PERMISS�O NEGADA!");
				}
	...
	public function addarquivoAction() {
		...
		if ($this->_request->isPost()) {
			...
			if ($contador) {
				...
				$this->_flashMessage("FALHA AO EXECUTAR OPERA��O, NENHUM ARQUIVO IMPORTADO!");
			}
    ...
    public function relatorioAction() {
        ...
        if ($dados["filtro_opcao"]) {
            if ($dados["filtro_opcao"] == "R") { $array["Op��o de Filtro"] = "A RECEBER"; }
            if ($dados["filtro_opcao"] == "S") { $array["Op��o de Filtro"] = "DOCUMENTOS QUE EST�O NO SETOR"; }
            if ($dados["filtro_opcao"] == "P") { $array["Op��o de Filtro"] = "DOCUMENTOS QUE PERTENCEM AO SETOR"; }
        }
        ...
        if ($dados["filtro_numero"]) {
            $array["N�mero"] = $dados["filtro_numero"];
        }
        if ($dados["filtro_ano"]) {
        ...
        if ($dados["filtro_funcionario"]) {
            $array["Funcion�rio"] = $dados["filtro_funcionario"];
        }
    ...
    public function encaminhamultiplaAction() {
        try {
            ...
            if (!$ids) {
                throw new Exception("Falha ao Executar Opera��o, Nenhum Documento Informado para Encaminhamento!");
            }
            ...
            $func = $tb->pegaLogado();
            if (!$func) {
                throw new Exception("Falha ao Executar Opera��o, Usuario Nao e Funcionario!!");
            }
            ...
            if (!count($encaminhar)) {
                throw new Exception("Falha ao Executar Opera��o, Nenhum Documento Habilitado para Encaminhamento!!");
            }
            
            if (count($erros)) {
                $html = "Os Seguintes Documentos n�o podem ser Encaminhados: ";
            ...
            if (!$lotacao) {
                throw new Exception("Falha ao Executar Opera��o, Nenhuma Lota��o Encontrada!");
            }
    ...
    public function salvaencaminhamultiplaAction() {
        ...
        try {
            ...
            if (!$this->_request->isPost()) {
                throw new Exception("Falha ao Executar Opera��o, Nenhuma Informa��o Recebida!");
            }
            
            $tb = new TbFuncionario();
            $func = $tb->pegaLogado();
            if (!$func) {
                throw new Exception("Falha ao Executar Opera��o, Nenhum Usu�rio Logado!");
            }
            
            $lotacao = $func->pegaLotacaoAtual();
            if (!$lotacao) {
                throw new Exception("Falha ao Executar Opera��o, Nenhuma Lota��o Dispon�vel para o Usu�rio!");
            }
            
            $ids = $this->_request->getPost("id_documento");
            
            if (!(is_array($ids) && count($ids))) {
                throw new Exception("Falha ao Executar Opera��o, Nenhum Documento para Encaminhar!");
            }
            
            $dados = array();
            $dados["tipo_destino"] = $this->_request->getPost("tipo_destino");
            if (!$dados["tipo_destino"]) {
                throw new UnexpectedValueException("Falha ao Executar Opera��o, Destino Inv�lido!");
            }
            ...
            $mt = $tb->getPorChave("E");
            if (!$mt) {
                throw new UnexpectedValueException("Falha ao Executar Opera��o, Status de Encaminhamento N�o Dispon�vel!");
            }
            
            $dados["despacho"] = $this->_request->getPost("despacho");
            
            if (!$dados["despacho"]) {
                throw new UnexpectedValueException("Falha ao Executar Opera��o, Nenhum Resumo Informado!");
            }
