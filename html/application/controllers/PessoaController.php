...
class PessoaController extends Escola_Controller_Logado {
    ...
    public function dadosAction() {
        try {
            ...
            if (isset($dados["cpf"]) && $dados["cpf"] && Escola_Util::limparNumero($dados["cpf"])) {
                $cpf = Escola_Util::limparNumero($dados["cpf"]);
            }
            ...
            if (!$id_pf && !$cpf) {
                throw new Exception("Falha ao Executar Opera��o, Nenhuma Pessoa F�sica Localizada!");
            }
            if ($id_pf) {
                $pf = $tb->getPorId($id_pf);
            } elseif ($cpf) {
                $pf = $tb->getPorCPF($dados["cpf"]);
            }
            ...
            $obj->id_pessoa_fisica = $pf->getId();
            $obj->cpf = utf8_encode(Escola_Util::formatCpf($pf->cpf));
            $obj->nome = utf8_encode($pf->nome);
            $obj->email = utf8_encode($pessoa->email);
            $obj->id_estado_civil = $pf->id_estado_civil;
            $obj->data_nascimento = Escola_Util::formatData($pf->data_nascimento);
            $obj->identidade_numero = utf8_encode($pf->identidade_numero);
            $obj->identidade_orgao_expedidor = utf8_encode($pf->identidade_orgao_expedidor);
            $obj->identidade_id_uf = $pf->identidade_id_uf;

            $obj->nome_pai = $pf->nome_pai;
            $obj->nome_mae = $pf->nome_mae;
            ...
            $obj->endereco->numero = utf8_encode($endereco->numero);
            $obj->endereco->complemento = utf8_encode($endereco->complemento);
            $obj->endereco->cep = utf8_encode(Escola_Util::formatCep($endereco->cep));
            ...
            $this->view->erro = utf8_encode($ex->getMessage());
        }
    }
    ...
    public function listarporpaginaAction() {
        ...
        if ($registros && count($registros)) {
            ...
            foreach ($registros as $registro) {
                ...
                $obj->cpf = utf8_encode(Escola_Util::formatCPF($registro->cpf));
                $obj->nome = utf8_encode($registro->nome);
                $items[] = $obj;
            }
    ...
    public function salvarAction() {
        try {
            ...
            if (isset($dados["flag_errors"])) {
                $flag_errors = (boolean) $dados["flag_errors"];
            } else {
                $flag_errors = false;
            }
            ...
            $this->view->id = $id;
            $this->view->descricao = utf8_encode($pessoa->toString());
        } catch (Exception $ex) {
            $this->view->erro = utf8_encode($ex->getMessage());
        }
    }
    ...
    public function pjlistarporpaginaAction() {
        ...
        if ($registros && count($registros)) {
            ...
            foreach ($registros as $registro) {
                $obj = new stdClass();
                $obj->id = $registro->getId();
                $obj->cnpj = utf8_encode(Escola_Util::formatCNPJ($registro->cnpj));
                $obj->sigla = utf8_encode($registro->sigla);
                $obj->razao_social = utf8_encode($registro->razao_social);
                $obj->nome_fantasia = utf8_encode($registro->nome_fantasia);
                $items[] = $obj;
            }
            $this->view->items = $items;
        }
    ...
    public function pjsalvarAction() {
        ...
        if (isset($dados["cnpj"]) && $dados["cnpj"]) {
            ...
            if (!$pessoa) {
                ...
                if ($errors) {
                    $this->view->erro = utf8_encode(implode("<br>", $errors));
                } else {
                    try {
                        $id = $pessoa->save();
                    } catch (Exception $e) {
                    ...
                    if ($id) {
                        $this->view->id = $id;
                        $this->view->cnpj = utf8_encode(Escola_Util::formatCnpj($pessoa->cnpj));
                        $this->view->sigla = utf8_encode($pessoa->sigla);
                        $this->view->nome_fantasia = utf8_encode($pessoa->nome_fantasia);
                        $this->view->descricao = utf8_encode(Escola_Util::formatCnpj($pessoa->cnpj) . " - " . $pessoa->nome_fantasia);
                    } else {
                        $this->view->erro = utf8_encode("Falha ao Executar Opera��o, Chame o Administrador!");
                    }
                }
            } else {
                $this->view->erro = utf8_encode("Pessoa J� Cadastrada!");
            }
        } else {
            $this->view->erro = utf8_encode("Campo C.N.P.J. Obrigat�rio!");
        }
    }
    ...
    public function pessoalistarporpaginaAction() {
        ...
        if ($registros && count($registros)) {
            ...
            foreach ($registros as $registro) {
                ...
                $obj->tipo = utf8_encode($registro->findParentRow("TbPessoaTipo")->toString());
                $obj->documento = utf8_encode($registro->mostrar_documento());
                $obj->nome = utf8_encode($registro->mostrar_nome());
                $items[] = $obj;
            }
            $this->view->items = $items;
        }
    ...
    public function viewAction() {
        ...
        if ($id) {
            ...
            if ($registro) {
                ...
                $button = Escola_Button::getInstance();
                $button->setTitulo("VISUALIZAR SERVI�O");
                ...
                $button->addFromArray(array("titulo" => "Excluir",
                    ...
                    "class" => "link_excluir",
                    "params" => array("id" => $id)));
                $button->addFromArray(array("titulo" => "Telefones",
                    "controller" => $this->_request->getControllerName(),
                    "action" => "telefone",
                    "img" => "icon-phone",
                    "params" => array("id_pessoa" => $id, "id" => 0)));
                $button->addFromArray(array("titulo" => "Informa��es Banc�rias",
                    "controller" => $this->_request->getControllerName(),
                    "action" => "ib",
                    "img" => "icon-money",
                    "params" => array("id_pessoa" => $id, "id" => 0)));
    ...
    public function telefoneAction() {
        ...
        if ($id_pessoa) {
            ...
            if ($pessoa) {
                ...
                $this->_flashMessage("FALHA AO EXECUTAR OPERA��O, NENHUMA PESSOA INFORMADA!");
                $this->_redirect($this->_request->getControllerName() . "/index");
            }
        } else {
            $this->_flashMessage("FALHA AO EXECUTAR OPERA��O, NENHUMA PESSOA INFORMADA!");
    ...
    public function editartelefoneAction() {
        ...
        if ($registro) {
            if (!$registro->telefone()) {
                $this->_flashMessage("REGISTRO DE TELEFONE INV�LIDO!");
                $this->_redirect($this->_request->getControllerName() . "/telefone/id/0/id_pessoa/{$registro->id_pessoa}");
            }
    ...
    public function ibAction() {
        ...
        if ($id_pessoa) {
            ...
            if ($pessoa) {
                ...
                $this->view->pessoa = $pessoa;
                $button = Escola_Button::getInstance();
                $button->setTitulo("CADASTRO DE PESSOA > INFORMA��ES BANC�RIAS");
                $button->addFromArray(array("titulo" => "Adicionar",
                    "controller" => $this->_request->getControllerName(),
                    "action" => "editarib",
                    "img" => "icon-plus-sign",
    ...
    public function viewibAction() {
        ...
        if ($id) {
            ...
            if ($registro) {
                ...
                $button = Escola_Button::getInstance();
                $button->setTitulo("VISUALIZAR PESSOA > INFORMA��O BANC�RIA");
                $button->addFromArray(array("titulo" => "Alterar",
                    "controller" => $this->_request->getControllerName(),
                    "action" => "editarib",
                    "img" => "icon-cog",
    ...
    public function editaribAction() {
        ...
        if ($registro) {
            if (!$registro->pessoa()) {
                $this->_flashMessage("REGISTRO DE INFORMA��ES BANC�RIAS INV�LIDO!");
        ...
        if ($this->view->ib->getId()) {
            $button->setTitulo("CADASTRO DE PESSOA > INFORMA��ES BANC�RIAS - ALTERAR");
        } else {
            $button->setTitulo("CADASTRO DE PESSOA > INFORMA��ES BANC�RIAS- INSERIR");
        }
        $button->addScript("Salvar", "salvarFormulario('formulario')", "icon-save");
        $button->addFromArray(array("titulo" => "Voltar",
            "controller" => $this->_request->getControllerName(),
        ...
        if ($pessoa) {
            ...
            if ($pt && $pt->pf()) {
                ...
                if ($pf) {
                    ...
                    $this->_flashMessage("DADOS INV�LIDOS!");
                    $this->_redirect($this->_request->getControllerName() . "/index");
                }
            } else {
                $this->_flashMessage("TIPO DE PESSOA INV�LIDO!");
                $this->_redirect($this->_request->getControllerName() . "/index");
            }
        } else {
            $this->_flashMessage("NENHUMA INFORMA��O RECEBIDA!");
    ...
    public function setfotoAction() {
        ...
        try {
	        ...
	        if (!$id_pessoa) {
	        		$ex = new IntranetException("Nenhuma Informa��o Recebida!");
			...
			if (!$arquivo->eImagem()) {
				$ex = new IntranetException("Arquivo n�o � uma Imagem!");
				$ex->setCaminho($this->_request->getControllerName() . "/foto/id/{$pessoa->getId()}");
				throw $ex;
			}
			...
			$arquivo->save();
			if (!$arquivo->getId()) {
				$ex = new IntranetException("Falha ao Executar Opera��o, Chame o Administrador!");
				$ex->setCaminho($this->_request->getControllerName() . "/foto/id/{$pessoa->getId()}");
				throw $ex;
			}
    ...
    public function webcamAction() {
        ...
        if ($id) {
            ...
            if ($pessoa) {
                ...
                $tb = new TbArquivo();
                $arquivo = $tb->createRow();
                $arquivo->setFromArray(array("legenda" => "FOTO PESSOA",
