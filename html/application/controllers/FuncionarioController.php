...
class FuncionarioController extends Escola_Controller_Logado {
    ...
    public function listarporpaginaAction() {
        ...
        if ($registros && count($registros)) {
            ...
            foreach ($registros as $registro) {
                ...
                $obj->matricula = utf8_encode($registro->matricula);
                $obj->cargo = utf8_encode($registro->findParentRow("TbCargo")->toString());
                $obj->setor = utf8_encode($registro->pegaLotacaoPrincipal()->findParentRow("TbSetor")->toString());
                $items[] = $obj;
            }
    ...
    public function indexAction() {
        ...
        $button->addFromArray(array("titulo" => "Pesquisar",
            ...
            "img" => "icon-search",
            "params" => array("id" => 0)));
        $button->addFromArray(array("titulo" => "Folha de Ponto",
            "onclick" => "folha_ponto()",
            "img" => "icon-time",
            "params" => array("id" => 0)));
    ...
    public function viewAction() {
        ...
        if ($registro->getId()) {
            ...
            $button = Escola_Button::getInstance();
            $button->setTitulo("VISUALIZAR FUNCION�RIO");
    ...
    public function addfuncionarioAction() {
        ...
        if ($funcionario || isset($dados["jan_cpf"])) {
            ...
            if ($this->view->funcionario->getId()) {
                $button->setTitulo("CADASTRO DE FUNCION�RIO - ALTERAR");
            } else {
                $button->setTitulo("CADASTRO DE FUNCION�RIO - INSERIR");
            }
    ...
    public function viewlotacaoAction() {
        ...
        if ($registro->getId()) {
            $this->view->registro = $registro;
            $button = Escola_Button::getInstance();
            $button->setTitulo("VISUALIZAR LOTA��O");
    ...
    public function editarlotacaoAction() {
        ...
        if ($this->_request->isPost()) {
            ...
            if ($errors) {
                ...
                if ($id_lotacao) {
                    ...
                    $this->view->actionErrors = array("FALHA AO EXECUTAR OPERA��O, CHAME O ADMINISTRADOR!");
                }
            }
        }
        $this->view->registro = $registro;
        ...
        $button = Escola_Button::getInstance();
        if ($this->view->registro->getId()) {
            $button->setTitulo("CADASTRO DE LOTA��O - ALTERAR");
        } else {
            $button->setTitulo("CADASTRO DE LOTA��O - INSERIR");
        }
        $button->addScript("Salvar", "salvarFormulario('formulario')", "icon-save");
        ...
        if ($id) {
            ...
            if ($registro) {
                ...
                if (!$registro->findParentRow("TbLotacaoTipo")->normal()) {
                    ...
                    $this->_flashMessage("LOTA��O PRINCIPAL N�O PODE SER EXCLU�DA!");
                }
            } else {
                $this->_flashMessage("INFORMA��O RECEBIDA INV�LIDA!");
            }
    ...
    public function listarAction() {
        ...
        if ($id) {
            ...
            if ($funcionario) {
                ...
                $func->nome = utf8_encode($pf->nome);
                $func->email = utf8_encode($pf->pega_pessoa()->email);
                $func->matricula = utf8_encode($funcionario->matricula);
                $func->cargo = utf8_encode($funcionario->findParentRow("TbCargo")->toString());
                $func->lotacao = utf8_encode($funcionario->pegaLotacaoPrincipal()->findParentRow("TbSetor")->toString());
                $func->telefone = utf8_encode($pf->pega_pessoa()->mostrarTelefones());
                $func->foto = utf8_encode($pf->mostrarFoto(80, "right", true));
            }
        }
    ...
    public function searchAction() {
        ...
        $button = Escola_Button::getInstance();
        $button->setTitulo("BUSCA DE FUNCION�RIOS");
        $button->addFromArray(array("titulo" => "Pesquisar",
            "onclick" => "pesquisar()",
            "img" => "icon-search",
            "params" => array("id" => 0)));
    ...
    public function viewarquivoAction() {
        ...
        if ($id) {
            ...
            if ($registro) {
                ...
                $button = Escola_Button::getInstance();
                $button->setTitulo("VISUALIZA��O DE ARQUIVO DE FUNCION�RIO");
                $button->addFromArray(array("titulo" => "Voltar",
                    "controller" => $this->_request->getControllerName(),
                    "action" => "arquivo",
                    "img" => "icon-reply",
    ...
    public function editararquivoAction() {
        ...
        $this->view->documento = $documento;
        $button = Escola_Button::getInstance();
        $button->setTitulo("CADASTRO DE ARQUIVO DE FUNCION�RIO");
        $button->addScript("Salvar", "salvarFormulario('formulario')", "icon-save");
    ...
    public function importarAction() {
        ...
        if ($arquivo && $arquivo["size"]) {
            ...
            $filename = ROOT_DIR . $path_arquivo . "/tmp_import_file.csv";
            $f = fopen($filename, "w+");
    ...
    public function editarocorrenciaAction() {
        ...
        $button = Escola_Button::getInstance();
        if ($this->view->registro->getId()) {
            $button->setTitulo("CADASTRO DE OCORR�NCIA - ALTERAR");
        } else {
            $button->setTitulo("CADASTRO DE OCORR�NCIA - INSERIR");
        }
        $button->addScript("Salvar", "salvarFormulario('formulario')", "icon-save");
    ...
    public function viewocorrenciaAction() {
        ...
        if ($registro) {
            $this->view->registro = $registro;
            $button = Escola_Button::getInstance();
            $button->setTitulo("VISUALIZAR OCORR�NCIA");
    ...
    public function imprimirAction() {
        ...
        if (count($id_funcionarios)) {
            if (isset($dados["ano_mes"]) && $dados["ano_mes"]) {
                ...
                $this->_flashMessage("FALHA AO EXECUTAR OPERA��O, DADOS INV�LIDOS1!");
            }
        } else {
            $this->_flashMessage("FALHA AO EXECUTAR OPERA��O, DADOS INV�LIDOS2!");
        }
        $this->_redirect($this->_request->getControllerName() . "/index");
    }
    ...
    public function baixarAction() {
        ...
        if ($funcionario) {
            ...
            if ($registros && count($registros)) {
                $path_tmp = ROOT_DIR . $path_arquivo . "/tmp/{$funcionario->getId()}/";
