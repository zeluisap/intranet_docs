	...
	public function listarAction() {
		...
		if ($registros && count($registros)) {
			...
			foreach ($registros as $registro) {
				...
				$obj->tipo = utf8_encode($registro->findParentRow("TbInfoTipo")->toString());
				$obj->titulo = utf8_encode($registro->titulo);
				$items[] = $obj;
			}
		...
		if ($this->_request->isPost()) {
			...
			if ($info) {
				if ($info->getId() != $dados["jan_id_info"]) {
					$tb = new TbInfoRef();
					$row = $tb->createRow();
					$row->setFromArray(array("id_info" => $info->getId(),
											 "tipo" => "I",
