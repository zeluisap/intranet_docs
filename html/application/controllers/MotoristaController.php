    ...
    public function listarporpaginaAction() {
        ...
        if ($registros && count($registros)) {
            ...
            foreach ($registros as $registro) {
                ...
                $obj->cnh_numero = utf8_encode($pm->cnh_numero);
                $obj->cnh_categoria = utf8_encode($cnh_categoria->toString());
                $obj->cnh_validade = utf8_encode(Escola_Util::formatData($pm->cnh_validade));
                $items[] = $obj;
            }
        ...
        if ($id) {
            ...
            if ($registro) {
                ...
                $button->addFromArray(array("titulo" => "Excluir",
                    ...
                    "class" => "link_excluir",
                    "params" => array("id" => $id)));
                $button->addFromArray(array("titulo" => "Serviços",
                    "controller" => $this->_request->getControllerName(),
                    "action" => "licenca",
                    "img" => "icon-copy",
                    "params" => array("id" => 0,
                        "id_motorista" => $id)));
                $button->addFromArray(array("titulo" => "Relatórios",
                    "controller" => $this->_request->getControllerName(),
                    "action" => "relatorio",
                    "img" => "icon-print",
                    "params" => array("id" => 0,
    ...
    public function licencaAction() {
        ...
        if ($id_motorista) {
            ...
            if ($motorista) {
                ...
                $button->addFromArray(array("titulo" => "Boleto Único",
                    "onclick" => "boleto()",
