	...
	public function identifyAction() {
		if ($this->getRequest()->isPost()) {
			...
			if (empty($formData["login_cpf"]) || empty($formData["login_senha"])) {
				$this->_flashMessage("CPF ou Senha em branco.");
			} else {
				$tb_usuario = new TbUsuario();
				$usuarios = $tb_usuario->getPorCPF($formData["login_cpf"]);
				if (!$usuarios) {
					$this->_flashMessage("Usuário não localizado.");
				} else {
					if ($usuarios->ativo()) {
						$tb = new TbPacote();
						...
						if ($pacotes) {
							...
							if (!$result->isValid()) {
								$this->_flashMessage("Falha ao efetuar login. Usuário ou Senha inválida!");
							} else {
							...
							$this->_flashMessage("Falha ao Executar Login, Nenhum pacote definido para o usuário!");
						}
					} else {
						$this->_flashMessage("USUÁRIO INATIVO!");
					}
				}
			}
		}
	...
	public function sistemaAction() {
		...
		if ($this->_request->isPost()) {
			...
			if (count($erros)) {
				...
				$dados_pessoa = array("id_pessoa_tipo" => $pt->id_pessoa_tipo,
				...
				if (!$msg) {
					...
					if ($id_pessoa) {
						$dados_pj = array("id_pessoa" => $id_pessoa,
						...
						$id_pj = $pj->save();
						$dados_sistema = array("sigla" => $dados["sistema_sigla"],
						...
						if (!$msg) {
							...
							*/
							...
							$dados_pessoa = array("id_pessoa_tipo" => $pt->getId(),
							...
							if (!$erros) {
								...
								if ($id_pessoa) {
									...
									if ($municipios) {
										...
										$municipio->setFromArray(array("descricao" => "MACAPA",
																	   "id_uf" => $uf->getId()));
										$municipio->save();
									}
									$pfs = new TbPessoaFisica();
									$pf = $pfs->createRow();
									$flag = array("id_pessoa" => $id_pessoa,
									...
									if (!$erros) {
										...
										if ($id_pf) {
											...
											if ($us->getId()) {
												...
												$usuario->setFromArray(array("id_pessoa_fisica" => $id_pf,
	...
	public function recuperasenhaofflineAction() {
		if ($this->_request->isPost()) {
			...
			if ($flag) {
				...
				$pfs = $tb->listar(array("filtro_cpf" => $this->_request->getPost("cpf"),
				...
				if ($pfs) {
					...
					if ($usuarios) {
						...
						if (($pf->pega_pessoa()->email == $this->_request->getPost("email")) &&
