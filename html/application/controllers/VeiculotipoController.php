<?php 
class VeiculotipoController extends Escola_Controller_Logado {
	...
	public function listarAction() {
		...
		if ($registros && $registros->count()) {
			...
			foreach ($registros as $registro) {
				$obj = new stdClass();
				$obj->id = $registro->id_veiculo_tipo;
	...
	public function indexAction() {
		...
		$button->setTitulo("TIPOS DE VEÍCULO");
	...
	public function viewAction() {
		...
		if ($id) {
			...
			if ($registro) {
				$this->view->registro = $registro;
				$button = Escola_Button::getInstance();
				$button->setTitulo("VISUALIZAR TIPO DE VEÍCULO");
