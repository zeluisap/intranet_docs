...
class VeiculoController extends Escola_Controller_Logado {
    ...
    public function salvarAction() {
        ...
        if (isset($dados["chassi"]) && $dados["chassi"]) {
            ...
            $errors = $registro->getErrors($flag_errors);
            if ($errors) {
                ...
                if ($id) {
                    $this->view->id = $id;
                    $this->view->descricao = utf8_encode($registro->toString());
                } else {
                    $this->view->erro = utf8_encode("Falha ao Executar Operação, Chame o Administrador!");
                }
            }
        } else {
            $this->view->erro = utf8_encode("Campo Chassi Obrigatório!");
        }
    }    
        ...
        if ($flag_chassi || $flag_placa) {
            ...
            if ($flag_chassi) {
                ...
                if ($rs && count($rs)) {
                    ...
                    if ($flag_placa && ($registro->placa != $dados["placa"])) {
                        $this->view->erro = utf8_encode("Placa Informada diferente da cadastrada para este Chassi!");
                        $flag = false;
                    }
                } elseif ($flag_placa) {
                    ...
                    if ($rs && count($rs)) {
                        ...
                        if ($flag_chassi && ($registro->chassi != $dados["chassi"])) {
                            $this->view->erro = utf8_encode("Chassi Informado é diferente do cadastrado para Esta Placa!");
                            $flag = false;
                        }
                    }
                }
            ...
            if ($flag) {
                ...
                foreach ($registro->toArray() as $field => $value) {
                    $obj->$field = utf8_encode($value);
                }
                $obj->data_aquisicao = Escola_Util::formatData($obj->data_aquisicao);
                $this->view->obj = $obj;
            }
        } else {
            $this->view->erro = utf8_encode("Informe o Número do Chassi e/ou Placa!");
        }
    }
	...
	public function listarporpaginaAction() {
		...
		if ($registros && count($registros)) {
			...
			foreach ($registros as $registro) {
                ...
                $obj->placa = utf8_encode($registro->placa);
                $obj->ano_modelo = $registro->ano_modelo;
                $obj->uf = utf8_encode($registro->findParentRow("TbUf")->toString());
				$obj->veiculo_tipo = utf8_encode($registro->findParentRow("TbVeiculoTipo")->toString());
                $obj->fabricante = utf8_encode($registro->findParentRow("TbFabricante")->toString());
                $obj->combustivel = utf8_encode($registro->findParentRow("TbCombustivel")->toString());
                $obj->veiculo = utf8_encode($registro->toString());
				$items[] = $obj;
			}
