...
class Documento extends Escola_Entidade {
    ...
    public function pega_arquivo() {
        if ($this->getId()) {
            ...
            $rg = $tb->fetchAll(" tipo = 'A' and id_documento = " . $this->getId());
            if ($rg && $rg->count()) {
    ...
    public function getErrors() {
        ...
        if (!$this->id_documento_tipo) {
            $msgs[] = "CAMPO TIPO DE DOCUMENTO OBRIGAT�RIO!";
        }
        ...
        if ($dt) {
            ...
            if ($dtt) {
                if ($this->getId() && $dtt->normal()) {
                    if (!trim($this->numero)) {
                        $msgs[] = "CAMPO N�MERO OBRIGAT�RIO!";
                    }
                    if (!$this->ano) {
                        $msgs[] = "CAMPO ANO OBRIGAT�RIO!";
                    }
                }
                ...
                if ($dm) {
                    ...
                    if ($dtt->normal() && $dm->normal() && !$prioridade) {
                        $msgs[] = "CAMPO PRIORIDADE � OBRIGAT�RIO PARA O MODO DE DOCUMENTO NORMAL!";
                    }
                }
            }
        }
        if ($this->eAdministrativo() || $this->pessoal()) {
            if ($this->eAdministrativo() && $this->possui_numero()) {
                if (!$this->numero) {
                    $msgs[] = "CAMPO N�MERO OBRIGAT�RIO!";
                }
                if (!$this->ano) {
            ...
            if (!trim($this->localizacao_fisica)) {
                $msgs[] = "CAMPO LOCALIZA��O F�SICA OBRIGAT�RIO PARA DOCUMENTOS ADMINISTRATIVOS!";
            }
        }
        if (!$this->pessoal() && !trim($this->resumo)) {
            $msgs[] = "CAMPO RESUMO OBRIGAT�RIO!";
        }
        if (!$this->id_documento_status) {
            $msgs[] = "CAMPO STATUS DE DOCUMENTO OBRIGAT�RIO!";
        }
        if (!$this->id_documento_modo) {
            $msgs[] = "CAMPO MODO DO DOCUMENTO OBRIGAT�RIO!";
        }
    ...
    public function mostrarNumero() {
        ...
        if ($dt) {
            $dtt = $dt->findParentRow("TbDocumentoTipoTarget");
            if ($dtt && !$dtt->pessoal() && $this->possui_numero()) {
    ...
    public function arquivar($dados = array()) {
        ...
        try {
            if ($this->arquivado()) {
                throw new Exception("FALHA AO EXECUTAR ARQUIVAMENTO, DOCUMENTO J� ARQUIVADO!");
            }
            ...
            $id = $mov->save();
            if (!$id) {
                throw new Exception("FALHA AO EXECUTAR OPERA��O, DOCUMENTO N�O ARQUIVADO!");
            }
            $tb = new TbDocumentoStatus();
            $ds = $tb->getPorChave("A");
            if ($ds) {
    ...
    public function cancelar_arquivar($dados = array()) {
        ...
        try {
            ...
            $id = $mov->save();
            if (!$id) {
                throw new Exception("FALHA AO EXECUTAR OPERA��O, CANCELAMENTO DE ARQUIVAMENTO FALHOU!");
            }
            $tb = new TbDocumentoStatus();
            $ds = $tb->getPorChave("E");
            $this->id_documento_status = $ds->getId();
    ...
    /*
     * Calcula o tempo que um documento est� no setor atual
     */
    ...
    /*
     * Verifica se o documento � CIRCULAR
     */
    ...
    /*
     * Verifica se o documento est� com o status EM TRAMITE
     */
    ...
    /*
     * Verifica se o Documento ultrapassou o per�odo para estada no setor atual
     */
    ...
    public function toForm(Zend_View_Abstract $view, $funcionario) {
        ...
        <script type="text/javascript">
            ...
            function atualizaFuncionario() {
                ...
                $('<tr><th width="50px">ID</th><th width="100px">Matr�cula</th><th>Nome</th><th>Cargo</th><th>Setor</th></tr>').appendTo($("#lista_interessado_resposta thead"));
                if (ajax_obj) {
                    ajax_obj.abort();
                }
                ajax_obj = $.ajax({
            ...
            function atualizaSetor() {
                ...
                var interessados = [];
                $(".field_setor").show();
                $('<tr><th width="50px">ID</th><th width="100px">Sigla</th><th>Descri��o</th></tr>').appendTo($("#lista_interessado_resposta thead"));
                if (ajax_obj) {
                    ajax_obj.abort();
                }
            ...
            <fieldset>
                ...
                <div class="control-group">
                    ...
                    <div class="controls">
                        ...
                        <?php if (!$this->getId()) { ?>
                            Para gerar um n�mero autom�tico, basta deixar esse campo em branco.
                        <?php } ?>
                    </div>
                </div>
                <div class="control-group">
                ...
                <?php if ($lotacao && $setor->protocolo()) { ?>
                    <div id="janela_procedencia_busca" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                        <div class="modal-header">
                            ...
                            <h3 id="myModalLabel">Localizar Proced�ncia</h3>
                        </div>
                        ...
                        <div class="modal-footer">
                            ...
                            <input type="button" value="Nova Proced�ncia" class="btn" id="bt_procedencia_novo" />
                    ...
                    <div id="janela_procedencia_novo" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">�</button>
                            <h3 id="myModalLabel">Nova Proced�ncia</h3>
                        </div>
                        <div class="modal-body">
                            ...
                            <fieldset>
                                ...
                                <div class="control-group">
                                    <label for="procedencia_descricao" class="control-label">Descri��o:</label>
                                    <div class="controls">
                                ...
                                <div class="control-group">
                                    ...
                                    <div class="controls">
                                        <select name="jan_tipo_interessado" id="jan_tipo_interessado">
                                            ...
                                            <option value="P">PESSOA F�SICA</option>
                                            <option value="J">PESSOA JUR�DICA</option>
                                            <option value="F" class="option_funcionario">FUNCION�RIO</option>
                                            <option value="S">SETOR</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="control-group field_funcionario">
                                    <label for="filtro_matricula" class="control-label">Matr�cula:</label>
                                    <div class="controls">
                    ...
                    <div id="janela_interessado_novo" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                        ...
                        <div class="modal-body">
                            ...
                            <fieldset>
                                ...
                                <div class="control-group field_setor">
                                    <label for="interessado_descricao" class="control-label">Descri��o:</label>
                                    <div class="controls">
                ...
                <div class="control-group">
                    <label for="procedencia" class="control-label">Proced�ncia:</label>
                    <div class="controls">
                        <div class="input-append">
                            ...
                            <?php if ($lotacao && $setor->protocolo()) { ?>
                                <div class="add-on">
                                    <a href="#" id="link_procedencia" title="Selecionar Proced�ncia">
            ...
            <?php if ($lotacao && $setor->protocolo()) { ?>
                <div class="well well-small" id="processo" style="display:none">
                    <fieldset>
                        ...
                        <div class="control-group">
                            <label for="processo_numero" class="control-label">N�mero / Ano:</label>
                            <div class="controls">
                                <input type="text" name="processo_numero" id="processo_numero" class="span2" /> / <input type="text" name="processo_ano" id="processo_ano" class="span1" />Para gerar um n�mero de Processo autom�tico, basta deixar esse campo em branco.
                            </div>
                        </div>
                    </fieldset>
                </div>
        ...
        <script type="text/javascript">
            ...
            function buscarDestinoSetor() {
                ...
                var ajax_obj = $.ajax({
                    ...
                    "success" : function(result) {
                        destinos = [];
                        $('<tr><th width="50px">ID</th><th width="160px">Sigla</th><th>Descri��o</th></tr>').appendTo($("#destino_resposta .head_destino"));
                        if (result.items && result.items.length) {
                            for (var i = 0; i < result.items.length; i++) {
                                var item = result.items[i];
                                destinos[i] = item;
            ...
            function buscarDestinoFuncionario() {
                ...
                var ajax_obj = $.ajax({
                    ...
                    "success" : function(result) {
                        destinos = [];
                        $('<tr><th width="50px">ID</th><th>Matr�cula</th><th>Nome</th><th>Cargo</th><th>Setor</th></tr>').appendTo($("#destino_resposta .head_destino"));
                        if (result.items && result.items.length) {
                            for (var i = 0; i < result.items.length; i++) {
                                var item = result.items[i];
                                destinos[i] = item;
                ...
                <div class="well well-small" id="destino_busca">
                    <fieldset>
                        ...
                        <div class="control-group filtro_busca filtro_funcionario linha_filtro">
                            <label for="destino_filtro_matricula" class="control-label">Matr�cula:</label>
                            <div class="controls">
