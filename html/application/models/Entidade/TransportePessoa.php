...
class TransportePessoa extends Escola_Entidade {
    ...
    public function toPDF($ss = false) {
        ...
        if (!$this->ativo()) {
            throw new UnexpectedValueException("Falha ao Executar Operação, Pessoa Inativa!");
        }

        $transporte = $this->getTransporte();
        if (!$transporte) {
            throw new UnexpectedValueException("Falha ao Executar Operação, Transporte Invalido!");
        }

        $tg = $transporte->getTransporteGrupo();
        if (!$tg) {
            throw new UnexpectedValueException("Falha ao Executar Operação, Grupo de Transporte Invalido!");
        }

        $tpt = $this->getTransportePessoaTipo();
        if (!$tpt) {
            throw new UnexpectedValueException("Falha ao Executar Operação, Tipo de Grupo de Transporte Invalido!");
        }

        $s = $ss->getServico();
        if (!$s) {
            throw new UnexpectedValueException("Falha ao Executar Operação, Nenhum Serviço Vinculado!");
        }
        ...
        if (!$rel) {
            throw new UnexpectedValueException("Falha ao Executar Operação, Relatorio Invalido!");
        }
    ...
    public function getCarteira() {
        $transporte = $this->getTransporte();
        if (!$transporte) {
            throw new Escola_Exception("Falha ao localizar Carteira de Permissionário!");
        }
        ...
        $params = array(":licenca_codigo" => "cp", //carteira de permissionario
        ...
        if (!$objs->count()) {
            throw new Exception("Falha ao Localizar Carteira de Permissionário!!!");
        }

        if ($objs->count() > 1) {
            //throw new Exception("Permissionário possui mais de uma Carteira Ativa!");
        }
