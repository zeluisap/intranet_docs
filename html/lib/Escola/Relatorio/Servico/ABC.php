...
class Escola_Relatorio_Servico_ABC extends Escola_Relatorio_Servico {
    ...
    public function toPDF() {
        ...
        <div class="normal">Permissionário/Concessionário: <?php echo $transporte->mostrar_codigo(); ?><br />
            ...
            CPF/CNPJ: <?php echo $proprietario_pf->mostrar_documento(); ?><br />
            Endereço: <?php echo $endereco->toString(); ?></div>
        <div></div>
        <div class="paragrafo">A <?php echo $pj->sigla; ?>-<?php echo $pj->mostrar_nome(); ?>, através desta autoriza o detentor da Permissão de Serviço <?php echo $tg->toString(); ?> a executar junto ao DETRAN-AP, o serviço de:<br />
        ...
        if ($pf) {
            ...
            ob_start();
            $this->css();
            $txt_nome_proprietario = $txt_ci = $txt_cpf = $txt_logradouro = $txt_numero = $txt_bairro = $txt_cidade = $txt_marca_modelo = $txt_placa = $txt_codigo = "";
            if ($transporte) {
                $txt_codigo = $transporte->mostrar_codigo();
            }
            if ($proprietario) {
                $txt_nome_proprietario = $proprietario->toString();
                $txt_ci = $proprietario_pf->mostrar_identidade();
                $txt_cpf = $proprietario_pf->mostrar_documento();
                if ($endereco) {
                    $txt_logradouro = $endereco->logradouro;
                    $txt_numero = $endereco->numero;
                    if ($bairro) {
                        $txt_bairro = $bairro->descricao;
                        $municipio = $bairro->findParentRow("TbMunicipio");
                        if ($municipio) {
                            $txt_cidade = $municipio->toString();
                            $uf = $municipio->findParentRow("TbUf");
                            if ($uf) {
                                $txt_cidade .= " - " . $uf->toString();
                            }
                        }
                    }
                }
            ...
            if ($veiculo) {
                ...
                if (trim($veiculo->modelo)) {
                    $flag[] = $veiculo->modelo;
                }
                if (count($flag)) {
                    $txt_marca_modelo = implode("/", $flag);
                }
                $txt_placa = $veiculo->placa;
                //proprietario do veiculo
                $txt_veiculo_proprietario = "";
                if ($proprietario) {
                    $proprietario_pessoa = $proprietario->findParentRow("TbPessoa");
                    if ($proprietario_pessoa) {
                        $pv = $veiculo->getProprietario();
                        if ($pv) {
                            $pv_pessoa = $pv->findParentRow("TbPessoa");
                            if ($pv_pessoa) {
                                if ($proprietario_pessoa->getId() != $pv_pessoa->getId()) {
                                    $txt_veiculo_proprietario = $pv_pessoa->toString();
                                }
                            }
                        }
                    }
            ...
            ?>
            <div></div>
            <div class="paragrafo declaracao" align="center">TERMO DE COMPROMISSO</div>
            <div></div>
            <div class="paragrafo">Eu, <strong><?php echo $txt_nome_proprietario; ?></strong>, portador da <strong>C.I.: <?php echo $txt_ci; ?></strong> e do <strong>C.P.F.: <?php echo $txt_cpf; ?></strong>, residente e domiciliado na <?php echo $txt_logradouro; ?>, No.: <?php echo $txt_numero; ?>, Bairro: <?php echo $txt_bairro; ?>, na cidade de <?php echo $txt_cidade; ?>, declaro que a partir desta data assumo a inteira responsabilidade de apresentar no prazo de <strong>72 horas</strong> o veículo <strong><?php echo $txt_marca_modelo; ?></strong> de placa <strong><?php echo $txt_placa; ?></strong><?php if ($txt_veiculo_proprietario) { ?>, de Proprietário: <?php echo $txt_veiculo_proprietario; } ?>, vinculado a <strong><?php echo $txt_codigo; ?></strong> para a vistoria na <?php echo $pj->mostrar_nome(); ?> - <?php echo $pj->sigla; ?>. O não comparecimento implicará na apreensão do veículo para medidas cabíveis.</div>
            <div></div><div></div>
            <div class="paragrafo">Cientifique-se,</div>
            <div></div><div></div>
            <div align="center">__________________________________________________<br />
                <strong><?php echo $txt_nome_proprietario; ?></strong></div>
            <div></div>
            <div class="direita"><?php echo $municipio; ?>-<?php echo $uf; ?>, <?php echo $dia; ?> de <?php echo $desc_mes; ?> de <?php echo $ano; ?>.</div>
            <?php
            $html = ob_get_contents();
