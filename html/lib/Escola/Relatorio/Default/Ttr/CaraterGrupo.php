...
class Escola_Relatorio_Default_Ttr_CaraterGrupo extends Escola_Relatorio_Default {
    ...
    public function toPDF() {
        ...
        $filename = $filter->filter(utf8_encode($this->relatorio->descricao));
    ...
    public function toXLS() {
        ...
        if ($stmt && $stmt->rowCount()) {
            $id_concessao_tipo = $linha = $coluna = 0;
            $breaks = array();
            while ($obj = $stmt->fetch(Zend_Db::FETCH_OBJ)) {
                if ($id_concessao_tipo != $obj->id_concessao_tipo) {
                    if ($id_concessao_tipo) {
                        $breaks[] = $linha;
                    }
                    $worksheet->writeString($linha, 0, $obj->concessao_tipo_descricao, $estilo_cabecalho);
                    $worksheet->mergeCells($linha, 0, $linha, 8);
                    $id_concessao_tipo = $obj->id_concessao_tipo;
                    $linha++;
                    $coluna = 0;
                    ...
                    $worksheet->writeString($linha, $coluna++, "Licença", $estilo_cabecalho);
                    $worksheet->writeString($linha, $coluna++, "Dt Licença", $estilo_cabecalho);
        ...
        <table class="table table-striped table-bordered">
            ...
            if ($stmt && $stmt->rowCount()) {
                ...
                while ($obj = $stmt->fetch(Zend_Db::FETCH_OBJ)) {
                    ...
                    <thead>
                        <tr>
                            <th colspan="11"><?php echo $obj->concessao_tipo_descricao; ?></th>
                        </tr>
                        <tr>
                            <th>Código</th>
                            <th>Nome</th>
                    ...
                    ?>
                    <tr>
                        <td><div class="text-center"><?php echo $transporte->codigo; ?></div></td>
                        <td><?php echo $proprietario_txt; ?></td>
                        <td><div class="text-center"><?php echo $obj->decreto; ?></div></td>
                        <td><div class="text-center"><?php echo Escola_Util::formatData($obj->concessao_data); ?></div></td>
                        <td><div class="text-center"><?php echo $data_cadastro; ?></div></td>
                        <td><div class="text-center"><?php echo $veiculo_txt; ?></div></td>
                        <td><div class="text-center"></div><?php echo $txt_modelo; ?></td>
                        <td><div class="text-center"></div><?php echo $ano; ?></td>
                        <td><div class="text-center"><?php echo $licenca_numero; ?></div></td>
                        <td><div class="text-center"><?php echo $licenca_data; ?></div></td>
                        <td><div class="text-center"><?php echo $licenca_validade; ?></div></td>
                    </tr>
            <?php } ?>
            <?php } else { ?>
                <tr>
