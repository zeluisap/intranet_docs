<?php
class Escola_Relatorio_Default_Ttr_Taxasapagar extends Escola_Relatorio_Default {
	...
	public function toXLS() {
        ...
        $worksheet = $workbook->addWorksheet("relatorio_taxasapagar");
        ...
        if ($stmt && $stmt->rowCount()) {
            ...
            while ($obj = $stmt->fetch(Zend_Db::FETCH_OBJ)) {
                ...
                $worksheet->writeString($linha, $coluna++, Escola_Util::formatData($obj->data_vencimento), $estilo_normal_centro);
		...
		$workbook->send("relatorio_Taxasapagar.xls");
		$workbook->close();
        die();
	}
        ...
        public function pegaStatement() {
            ...
            $sql = $db->select();
            $sql->from(array("svs" => "servico_solicitacao"), array("svs.data_vencimento"));
            ...
            if (isset($this->dados["data_inicio"]) && $this->dados["data_inicio"]) {
                ...
                $sql->where("svs.data_vencimento >= '{$data_inicio}'");
            }
            
            if (isset($this->dados["data_fim"]) && $this->dados["data_fim"]) {
                $data_fim = Escola_Util::montaData($this->dados["data_fim"]);
                $sql->where("svs.data_vencimento <= '{$data_fim}'");
            }
            
            
            $sql->group("tg.descricao");
            $sql->group("tg.id_transporte_grupo");
            $sql->group("svs.data_vencimento");
...
?>
            <tr>
                <td><div class="text-center"><?php echo Escola_Util::formatData($obj->data_vencimento); ?></div></td>
                <td><?php echo $filho->mostrar_nome(); ?></td>
                <td><div class="text-center"><?php echo Escola_Util::number_format($obj->valor_servico); ?></div></td>
            </tr>
<?php } ?>
        </tbody>
<?php if (count($resumo)) { ?>
...
<?php foreach ($resumo as $item_resumo) { ?>
            <tr>
                ...
                <td><div class="text-center"><div class="text-center"><?php echo Escola_Util::number_format($item_resumo->valor); ?></div></td>
            </tr>
<?php } ?>
        </body>
    </table>
